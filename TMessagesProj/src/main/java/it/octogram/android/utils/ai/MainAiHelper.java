...
import it.octogram.android.utils.ai.whisper.WhisperLocalHelper;
...
public class MainAiHelper {
    ...
    public static int getEnabledProvidersCount() {
        ...
        if (WhisperLocalHelper.isAvailable()) i++;
        return i;
    }
    ...
    public static AiProvidersDetails getPreferredProvider() {
        AiProvidersDetails favoriteProvider = GeminiHelper.isAvailable() ? AiProvidersDetails.GEMINI : ChatGPTHelper.isAvailable() ? AiProvidersDetails.CHATGPT : WhisperLocalHelper.isAvailable() ? AiProvidersDetails.WHISPER : AiProvidersDetails.OPENROUTER;
        ...
        } else if (recentProvider == AiProvidersDetails.WHISPER.getId() && WhisperLocalHelper.isAvailable()) {
    ...
    public static boolean isProviderAvailable(AiProvidersDetails preferredProvider) {
        if (preferredProvider == AiProvidersDetails.GEMINI) {
            ...
            return WhisperLocalHelper.isAvailable();
        } else {
    ...
    private static void request(AiPrompt aiPrompt, AiProvidersDetails preferredProvider, OnResultState state, boolean forced) {
        ...
        if (preferredProvider == AiProvidersDetails.GEMINI) {
            ...
            WhisperLocalHelper.prompt(aiPrompt, state);
        } else {
            OctoLogging.e(TAG, "Unknown provider: " + preferredProvider);
            state.onFailed();
        }
